      ...
      >
        <motion.div className="absolute inset-0" whileHover={{ scale: 1.06 }} transition={{ duration: 0.7, ease: [0.16, 1, 0.3, 1] }}>
          <Image
            ...
            fill
            sizes={item.featured ? "(min-width: 768px) 66vw, 100vw" : "(min-width: 768px) 33vw, 100vw"}
        ...
        <div className="absolute top-4 text-xs tracking-widest text-ronin-white/70 uppercase ltr:left-4 rtl:right-4">
          {String(index + 1).padStart(2, "0")}
        </div>

        <div className="absolute inset-x-0 bottom-0 p-5">
          <p className="text-xs tracking-widest text-ronin-red uppercase">{lang === "ar" ? item.tagAr : item.tagEn}</p>
          <h3 className="mt-1 font-display text-xl leading-tight font-semibold text-ronin-white transition-transform duration-500 group-hover:-translate-y-1">
