"use client";

import { AnimatePresence, motion } from "motion/react";
import { RONIN_MARK_PATH, RONIN_MARK_VIEWBOX } from "@/lib/motion/roninMarkPath";
import { useTransition } from "./TransitionContext";

/**
 * The visual half of the transition system — reads phase/seed from
 * TransitionContext (which also drives route-change auto-play and manual
 * triggers like the language switch) and renders the branded cover/hold/
 * reveal sequence. One instance, mounted once in SiteChrome.
 */
export function PageTransitionOverlay() {
  const { phase, seed } = useTransition();

  // Slight per-transition variation: entry side and glow origin alternate.
  const fromLeft = seed < 0.5;
  const glowX = 20 + seed * 60;
  const covered = phase === "holding" || phase === "revealing";

  return (
    <AnimatePresence>
      {phase !== "idle" && (
        <motion.div
          aria-hidden="true"
          data-testid="page-transition-overlay"
          className="pointer-events-none fixed inset-0 z-[500] overflow-hidden bg-ronin-black"
          initial={{ clipPath: fromLeft ? "inset(0 100% 0 0)" : "inset(0 0 0 100%)" }}
          animate={{
            clipPath:
              phase === "covering"
                ? "inset(0 0% 0 0)"
                : phase === "holding"
                  ? "inset(0 0% 0 0)"
                  : fromLeft
                    ? "inset(0 0 0 100%)"
                    : "inset(0 100% 0 0)",
          }}
          exit={{ opacity: 0, transition: { duration: 0.01 } }}
          transition={{
            duration: phase === "covering" ? 0.45 : phase === "revealing" ? 0.55 : 0,
            ease: [0.87, 0, 0.13, 1],
          }}
        >
          <motion.div
            className="absolute h-[70vmax] w-[70vmax] rounded-full bg-ronin-red/25 blur-[110px]"
            style={{ top: "50%", left: `${glowX}%`, transform: "translate(-50%, -50%)" }}
            animate={covered ? { opacity: [0.7, 1, 0.7], scale: [1, 1.08, 1] } : { opacity: 1, scale: 1 }}
            transition={{ duration: 1.4, repeat: covered ? Infinity : 0, ease: "easeInOut" }}
          />
          <div className="absolute inset-x-0 top-1/2 h-px -translate-y-1/2 bg-gradient-to-r from-transparent via-ronin-red/60 to-transparent" />

          <motion.div
            className="absolute top-1/2 left-1/2 h-24 w-24 -translate-x-1/2 -translate-y-1/2 text-ronin-red md:h-32 md:w-32"
            initial={{ scale: 0.5, opacity: 0, rotate: fromLeft ? -8 : 8 }}
            animate={{
              scale: phase === "covering" ? 1 : phase === "holding" ? [1, 1.04, 1] : 1.15,
              opacity: phase === "revealing" ? 0 : 1,
              rotate: 0,
            }}
            transition={
              phase === "holding"
                ? { duration: 1.2, repeat: Infinity, ease: "easeInOut" }
                : { duration: phase === "covering" ? 0.45 : 0.35, ease: [0.16, 1, 0.3, 1] }
            }
            style={{ filter: "drop-shadow(0 0 30px rgba(169,45,46,0.6))" }}
          >
            <svg viewBox={RONIN_MARK_VIEWBOX} className="h-full w-full">
              <path d={RONIN_MARK_PATH} fill="currentColor" />
            </svg>
          </motion.div>
        </motion.div>
      )}
    </AnimatePresence>
  );
}
